note
	description: "Summary description for {INT_KEY_TESTS}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"
...
feature  -- types
	...
	map: SORTED_MAP_ADT[INTEGER, PERSON]
		deferred
		end
...
feature  -- tests
	...
	t2: BOOLEAN
		do
			...
			check Result end
			map.remove(4)
			print("%NMap after remove 4: " + map.out)
			Result := map.out ~ "(2,c)(7,b)"
			check Result end
			map.remove (2)
			print("%NMap after remove 2: " + map.out)
			map.remove (7)
			print("%NMap after remove 7: ")
			Result := map.out ~ ""
			check Result end
		end
	...
	t7: BOOLEAN
		...
		do
			...
			Result := sub_map.out ~ "(-1,b)"
			check Result end
			sub_map := map.sub_map (-5, 7)
			Result := map ~ sub_map
			check Result end
		end
	...
	t9: BOOLEAN
		...
		do
			...
			map.extend (-4, "e")
			map.extend (0, "f")
			l_list := << [-4, "e"], [-1, "b"], [0, "f"], [1, "d"], [2, "a"], [5, "c"]>>
			Result :=
